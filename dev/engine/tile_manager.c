...
void engine_tile_manager_draw_candy( unsigned char type, unsigned char x, unsigned char y )
{
	...
	unsigned char offset = ( line - 1 ) * 2 * BASE_TILES_OFFSET + type * 2 + BASE_BLOCK_OFFSET;
	draw_tile( offset, x, y );
}
...
void engine_tile_manager_draw_bonus( unsigned char type, unsigned char x, unsigned char y, unsigned char multiplier )
{
	...
	offset += ( multiplier - 1 ) * BASE_TILES_OFFSET * 2;
	draw_tile( offset, x, y );

	//devkit_SMS_setNextTileatXY( x + 0, y + 0 );	devkit_SMS_setTile( ( *game_tiles__tilemap__bin + offset + 0 ) | priority );
	//devkit_SMS_setNextTileatXY( x + 1, y + 0 );	devkit_SMS_setTile( ( *game_tiles__tilemap__bin + offset + 1 ) | priority );
}
...
{
	unsigned char offset = ( 2 * type ) + 2 * BASE_TILES_OFFSET;
	draw_tile( offset, x, y );
}
